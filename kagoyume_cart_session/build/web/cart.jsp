...
<html>
    ...
    <body>
        ...
        <%if(ci != null && ci.cartContents() && !num.isEmpty()){%>
        <table border="1">
            <tr><td>サムネイル画像</td><td>商品名</td><td>価格</td><td>数量</td><td></td></tr>
            <%for(int number = 0; number < num.size(); number++){%>
                <tr>
                    <td><img src="<%=ci.getThumnail(num.get(number))%>" alt=""></td>
                    <td>
                        <a href="Item?itemID=<%=num.get(number)%>&previous=cart">
                            <%=ci.getName(num.get(number))%>
                        </a>
                    </td>
                    <td><%=ci.getPrice(num.get(number)) + "円"%></td>
                    <td><%=ci.getItemNumber(num.get(number)) + "個"%></td>
                    <td>
                        <form action="Cart" method="get">
                            <input type="submit" value="削除">
                            <input type="hidden" name="delete" value="<%=num.get(number)%>">
                        </form>
                    </td>   
                <tr>
            <%}%>
