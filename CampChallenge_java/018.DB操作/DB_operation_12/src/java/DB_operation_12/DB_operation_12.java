 ...
 * @author sumi3
 */
public class DB_operation_12 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DB_operation_12</title>");            
            out.println("</head>");
            out.println("<body>");
            ...
            try{
                ...
                ps = co.prepareStatement("SELECT * FROM profiles WHERE name LIKE ? OR age = ? OR birthday = ? ORDER BY profilesID ASC");
                String typename = request.getParameter("name");
                ps.setString(1, "%" + typename + "%");
                ps.setInt(2, Integer.valueOf(request.getParameter("age")));
                ps.setDate(3, Date.valueOf(request.getParameter("birthday")));
                
                
                rs = ps.executeQuery();
                if(rs.next() == false){
                    out.print("該当する人物データはありません");
                } else {
                    rs.beforeFirst();
                    out.print("<h1>複合検索で一致したデータ一覧</h1>");
                    while(rs.next()){
