 ...
 * @author sumi3
 */
public class DB_operation_10 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DB_operation_10</title>");            
            out.println("</head>");
            out.println("<body>");
            ...
            try{
                ...
                int number = Integer.valueOf(request.getParameter("profilesID"));
                ps = co.prepareStatement("DELETE FROM profiles WHERE profilesID = ?");
                ps.setInt(1, number);
                int eu = ps.executeUpdate();
                if (eu == 0){   
                    out.print("指定されたプロフィールIDに該当するデータは存在しません");
                } else {
